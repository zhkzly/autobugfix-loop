...
class OperatorRequest:
    ...
    def request_digest(self) -> str:
        return self.persisted_request_digest or digest_payload(self.payload())
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "OperatorRequest":
        ...
        if stored:
            ...
            for key in (
                ...
                legacy.pop(key)
            compatible = (
                item.budget_grant_id is None and stored == digest_payload(pre_budget)
            ) or (
                item.experiment_line_id is None
                and item.budget_grant_id is None
                and stored == digest_payload(legacy)
            )
            if stored != computed and not compatible:
