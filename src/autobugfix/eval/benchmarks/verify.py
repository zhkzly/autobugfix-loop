...
class Defects4JVerifierContract:
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.eligibility_receipt_digest) != 64 or any(
            ...
            raise BenchmarkContractError(
                "verifier eligibility receipt digest must be sha256"
            )
    ...
    def from_dict(cls, data: Mapping[str, Any]) -> "Defects4JVerifierContract":
        ...
        return cls(
            ...
            eligibility_receipt_digest=str(
                data.get("eligibility_receipt_digest") or ""
            ),
...
class Defects4JManagedVerifier:
    ...
    def run(
        ...
        *,
        ...
        try:
            passed, _, stdout, stderr, exit_code = run_official_verifier(
                worktree,
                contract,
...
def run_official_verifier(
    ...
    runtime = Defects4JRuntime(
        EvalBenchmarkConfig(
            ...
            command_timeout_seconds=contract.timeout_seconds,
            min_free_disk_gb=1,
    ...
    try:
        ...
        stdout = Path(evidence.stdout_path).read_text(encoding="utf-8", errors="replace")
        stderr = Path(evidence.stderr_path).read_text(encoding="utf-8", errors="replace")
    finally:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        passed, failures, stdout, stderr, exit_code = run_official_verifier(
            worktree,
            contract,
            run_dir,
        )
