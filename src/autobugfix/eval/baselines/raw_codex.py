...
from autobugfix.eval.benchmarks.issues import visible_problem_statement
...
class RawCodexBaselineService:
    ...
    def prepare(
        ...
        if runner.sdk_version != seed.sdk_version:
            raise RawCodexBaselineError("Raw SDK version differs from protocol")
    ...
    def _case_bundle(receipt: EligibilityReceipt) -> dict[str, Any]:
        problem, attachment_records = visible_problem_statement(receipt)
    ...
    def run_formal(
        ...
        *,
        ...
        safe_component(run_id, "run_id")
        prepared = self.store.read_prepared_raw_baseline_manifest(
